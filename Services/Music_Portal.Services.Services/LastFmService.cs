...
namespace Music_Portal.Services.Services
{
    public class LastFmService : ILastFmService
    {
        ...
        public async Task<IEnumerable<SimilarArtistLastFm>> GetSimilarArtists(string name)
        {
            var artistName = TrimAndReplaceSpacesToPluses(name);
            var httpClient = new HttpClient();
            var response = await httpClient.GetAsync(
                $"{Environment.GetEnvironmentVariable("REQUEST")}?method=artist.getsimilar&artist={artistName}" +
                $"&api_key={Environment.GetEnvironmentVariable("API_KEY")}&format=json");
            var result = await response.Content.ReadAsAsync<SimilarArtistsResponseLastFm>();
        ...
        public async Task<IEnumerable<AlbumTrackLastFm>> GetAlbumTracks(string album, string artist)
        {
            var albumName = TrimAndReplaceSpacesToPluses(album);
            var artistName = TrimAndReplaceSpacesToPluses(artist);
            var httpClient = new HttpClient();
